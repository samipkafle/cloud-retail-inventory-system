...
### Reporting still runs client-side, not as a dedicated Lambda

FR-08's CSV export is implemented as client-side JavaScript (`frontend/js/actions.js`) rather than the SAD-designed `GET /reports?from=&to=` API endpoint with S3-stored report files. FR-09's forecast has since been moved to a real `GET /forecast` Lambda (see the FR-09 row above) — reporting is the remaining piece still client-side only.
