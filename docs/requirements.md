...
| FR-10 | Train and display demand recommendations using historical sales patterns | Must | Backend implemented, not yet surfaced in the UI — a Python Lambda (`lambda-python/recommendation-engine/handler.py`) fits an ordinary-least-squares linear trend per product on a daily EventBridge schedule, writing to a new `RetailRecommendations` table; `GET /recommendations` serves it, ranked by predicted demand with a stated basis (days of history, R²). Trained on a seeded synthetic ~9-month sales history per the SAD report's own risk mitigation (see `scripts/seed-synthetic-sales.ts`). Not yet wired into the frontend — see the technology-stack deviation below for the scikit-learn→numpy substitution |
