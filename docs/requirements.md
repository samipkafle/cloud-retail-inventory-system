...
| --- | --- | --- | --- |
| FR-01 | Authenticate users and apply role-based access | Must | Implemented and enforced — the frontend signs in against the real Cognito User Pool (SRP flow via `amazon-cognito-identity-js`), attaches the ID token to every API call, and the API Gateway Cognito authorizer (`AUTH_ENABLED = true` in `cdk-stack.ts`) rejects unauthenticated requests. The `manager` Cognito group gates product create/update/delete in the handler; staff (any authenticated non-manager) can record sales and view inventory/alerts. Self-sign-up is disabled by design — a manager creates new staff/manager accounts from the app's Team page (`GET/POST /users`, `users-handler.ts`), which calls Cognito's `AdminCreateUser` and emails the temporary password directly to the new user; the API never sees or returns a password value. See [architecture.md](architecture.md#authentication-and-authorization-fr-01) and [testing.md](testing.md#fr-01-authentication-and-authorization--2026-09-06) for the implementation and verification detail |
...
| FR-10 | Train and display demand recommendations using historical sales patterns | Must | Implemented — a Python Lambda (`lambda-python/recommendation-engine/handler.py`) fits an ordinary-least-squares linear trend per product on a daily EventBridge schedule, writing to a new `RetailRecommendations` table; `GET /recommendations` serves it, ranked by predicted demand. The Insights page's "AI demand recommendations" section (`frontend/js/render.js`'s `renderAiRecommendations()`) displays each product's trend label and predicted demand alongside its stated basis (days of history, R²) — shown as decision support, not a directive, per the SAD report's AI ethics position. Trained on a seeded synthetic ~9-month sales history per the SAD report's own risk mitigation (see `scripts/seed-synthetic-sales.ts`). See the technology-stack deviation below for the scikit-learn→numpy substitution |
...
### Recommendation engine: numpy instead of scikit-learn

The SAD report specifies scikit-learn for FR-10's recommendation engine. scikit-learn has no public AWS Lambda layer for `ap-southeast-2` on a current Python runtime (checked against both [Klayers](https://github.com/keithrozario/Klayers), which publishes `pandas`/`numpy`/`scipy` for this region but not scikit-learn, and a smaller community project that only covers US regions), and this environment has no Docker available to build one from source. `lambda-python/recommendation-engine/handler.py` uses `numpy.polyfit` for the ordinary-least-squares regression instead — the same underlying math scikit-learn's `LinearRegression` uses (`numpy.linalg.lstsq`) for a single feature. pandas and scipy were tried first for the same job but their combined unzipped layer size exceeded Lambda's 250MB limit; numpy alone comfortably fits. The model itself (linear trend over daily sales, ranked by R²) matches the SAD design's intent — the deviation is the implementation library, not the approach.

One documented, honest limitation of the linear model: a product with a genuinely seasonal (cyclical) sales pattern doesn't fit well to a straight line, so it's reported as "Stable" rather than a spurious Increasing/Decreasing — a future iteration could add a seasonal/cyclical model, but that's out of scope for this linear-trend-v1 implementation.
